...
import { StyleSheet, TouchableHighlight, Animated, View, Text, Dimensions } from 'react-native';
...
export default class HabitListItem extends PureComponent {
  ...
  render() {
    ...
    if (!habit.intervals[this.state.habit.intervals.length - 1]) {
      ...
      recentCompletions = this.state.habit.intervals[this.state.habit.intervals.length - 1]
        .completions;
    }
    const completions = [];
    const indicators = [];
    for (let i = 0; i < this.state.habit.bonusFrequency; i += 1) {
      const completed = !(recentCompletions.length <= i);
      completions.push(<CompletionButton
        key={`CompletionButton${habit.id}${i.toString()}`}
        completed={completed}
        addCompletion={this.props.addCompletion}
        removeCompletion={this.props.removeCompletion}
        habit={habit}
      />);
      indicators.push(<View
        key={`Indicator${habit.id}${i.toString()}`}
        style={[styles.indicator, completed && styles.completed]}
      />);
    }
                  ...
                  <View style={styles.overlayContainer}>
                    <View style={styles.topRow}>
                      ...
                      <View style={styles.expandedPointValueContainer}>
                        <Text style={styles.pointValue}>
                          {habitUtil.returnDisplayInterval(this.state.habit)}
                        </Text>
                        <Text style={styles.pointValue}>
                          {habitUtil.returnPointValueString(this.state.habit)}
                        </Text>
                    ...
                    <View style={styles.completionRow}>{completions}</View>

                    <TouchableHighlight onPress={this._onPressEdit}>
                      <Text style={styles.editButton}>Edit Habit</Text>
                    </TouchableHighlight>
                  </View>
                </TouchableHighlight>
              </FadeInView>
